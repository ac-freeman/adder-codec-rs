...
///
/// Quantization tables copied from https://github.com/xiph/rav1e/blob/ef16a0a11862865b4f03213fc9f831fe8fd64d2e/src/quantize.rs (BSD 2-Clause License)
...
pub fn dc_q(qindex: u8, delta_q: i8, bit_depth: usize) -> i16 {
    static DC_Q: [&[i16; 256]; 3] = [&dc_qlookup_Q3, &dc_qlookup_10_Q3, &dc_qlookup_12_Q3];
...
pub fn ac_q(qindex: u8, delta_q: i8, bit_depth: usize) -> i16 {
    static AC_Q: [&[i16; 256]; 3] = [&ac_qlookup_Q3, &ac_qlookup_10_Q3, &ac_qlookup_12_Q3];
    let bd = ((bit_depth ^ 8) >> 1).min(2);
...
mod tests {
    ...
    // Note about DCT coefficients. The maximum value of a coefficient is the maximum value of the
    // input, times the number of pixels in the input. For example, if the input is 8x8 and 8-bit, the maximum
    // value of a coefficient is 255 * 64 = 16320.
    // For encoding coefficients of delta_t, we should maybe quantize (scale) the coefficients to the range
    // [-16320, 16320] before dividing by the quantization matrix. Then we can ensure that each coefficient
    // can fit within 8-bits.

    fn gen_array(dim: usize) -> Vec<f64> {
    ...
    fn quantize(qparam: Option<u8>) {
        ...
        for (idx, (new, old)) in arr_i16.iter().zip(pre_quantized.iter()).enumerate() {
            assert_eq!(new, old);
        }
