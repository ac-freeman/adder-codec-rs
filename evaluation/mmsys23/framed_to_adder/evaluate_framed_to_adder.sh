...
for i in "${!filenames[@]}"; do
    ...
    do
        ...
        cargo run --release --bin adder_simulproc -- --show-display 0 --scale 1.0 --input-filename "${DATASET_PATH}/${FILENAME}" --output-raw-video-filename "./tmp" --c-thresh-pos $i --c-thresh-neg $i --frame-count-max 0 --output-events-filename "./tmp_events.adder" --ref-time ${REF_TIME} --delta-t-max ${DTM} >> "${DATA_LOG_PATH}/${FILENAME}/${i}_${REF_TIME}.txt"
        cargo run --release --bin adderinfo -- -i "./tmp_events.adder" -d >> "${DATA_LOG_PATH}/${FILENAME}/${i}_${REF_TIME}.txt"
        ...
        rm -rf "./tmp.mp4"
        mv "./tmp_vmaf.json" "${DATA_LOG_PATH}/${FILENAME}/${i}_${REF_TIME}_vmaf.json"
    done
    echo "${FILENAME}"
done
